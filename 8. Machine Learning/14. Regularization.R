...
rmse_results <- bind_rows(rmse_results,
                          data.frame(method="Regularized Movie Effect Model",  
                                     RMSE = model_3_rmse ))
...
# Q2
schools %>% summarize(median = median(size))
schools %>% top_n(10, score) %>% summarize(median = median(size))

# Q3
schools %>% top_n(-10, score) %>% summarize(median = median(size))

# Q4
dat <- schools %>% mutate(top = ifelse(rank > 10, "N", "Y"))
qplot(score, size, data = dat, colour = top)
